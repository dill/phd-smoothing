...
Intuitivelyi, we are comparing the side lengths of the polygon and the approximation of the polygon in this iteration (\cite{snider}, \emph{p. A-3}), both scaled by the distance between the first two vertices (in their respective domains.)
...
Calculating the forwards map is simply a case of evaluating $f$ at the necessary points. If we wish to find the point on polygon ($w$), given we know the point on the disk ($w^*$), we compute:
...
\section{Software}
There are several software packages available to perform the computation of the map (\cite{driscoll}, \emph{p. 39}.) Two were tested for our purposes: \texttt{SCPACK} and the SC Toolbox for MATLAB.
