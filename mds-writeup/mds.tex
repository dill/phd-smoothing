...
Gower's method relies on taking a single new point $x_{\text{new}}$, and inserting at its distance from the original points ($\tilde{X}$) into a new dimension, orthogonal to $\tilde{X}$; the extra dimension is then be projected on to the old axes. The technical details of the computation are detailed first, then some of the issues with the method are discussed.
...
VVVVVVVVVVVVVVVVVVVV absorb this into the above
...
giving the representation seen in diagram BLAH

ANOTHER PLOT HERE PLEASE.
...
\begin{aligned}
\tilde{x}_{\text{new}} &= \frac{1}{2} \Lambda^{-1} \tr{\tilde{X}} \mathbf{d}\\
&=(0.3535, 0.2041, -4.3 \cross 10^{-17})
\end{aligned}
\end{equation}

This is clearly different from the coordinates in equation (\ref{origMDScoords}).
...
CLEAR UP NOTATION BETWEEN $\tilde{X}$ and $X$.
