...
\item Given a triple of vertices, $(v_i, v_{i+1}, v_{i+2})$ if the path $(v_i, v^1_1, \dots, v^1_j, v_{i+2})$ is shorter than the path $(v_i, v_{i+1}, v_{i+2})$ then replace $v_{i+1}$ with $v^1_1, \dots, v^1_j$ (\fig{wdia}, ($v$)). The $v^1_1$ and $v^1_j$ are the first and last intersections of the line between $v_i$ and $v_{i+2}$ and the boundary of $\Gamma$. The elements between $v^1_1$ and $v^1_j$ are the vertices of $\Gamma$ between $v^1_1$ and $v^1_j$ which have had superfluous vertices removed using the same scheme as in 2. For example in \fig{wdia} ($iv$), the path $(v_1, v_2, v_3)$ is longer than the path $(v_1, v^1_1, v^1_2, v^1_3, v_3)$ so the former is replaced with the latter in $\mathcal{P}$.
