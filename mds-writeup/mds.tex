...
Taking the unknown new locations and putting them in an $n \times p$ matrix, $X$. Taking $S=X\tr{X}$, and performing an eigen-decomposition we can see that $S=U\Lambda\tr{U}$. So we may now express $S$ as some set of length $n$ vectors (hopefully less than $p$ of them.) We now have that $\tilde{X}=U\Lambda^{1/2}$.
