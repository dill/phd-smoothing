...
Note that an additive constant can be computed and added to the non-diagonal entries of $D$ such that the eigenvalues of $S$ are non-negative.
...
Gower's paper shows that performing MDS on a dataset is equivalent to performing MDS on a reduced set of points and then inserting the remaining points. However, he only shows that this is true for the case when we consider Euclidean distance between the data. 
