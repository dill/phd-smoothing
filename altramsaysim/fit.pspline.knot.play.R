# playing around with the knot problem...
library(soap)
...
knots.sc<-list(v=seq(min(prediction.grid$v),max(prediction.grid$v)
               ,length.out=18),w=seq(min(prediction.grid$w),
...
#plot(mapped.data$w,mapped.data$v,asp=1,pch=".",cex=2,xlim=c(min(knots.sc$w),max(knots.sc$w)),ylim=c(min(knots.sc$v),max(knots.sc$v)))
#abline(v=knots.sc$w)
#abline(h=knots.sc$v)
   ### sc code
   # fit with sc
   ...
   b.mapped<-gam(y~te(v,w,bs="ps",m=pspline.order,k=c(length(knots.sc$v)-4,length(knots.sc$w)-4)),data=mapped.data,knots=knots.sc)
...
image(xm,yn,matrix(fv.mapped,m,n),col=heat.colors(100),xlab="v",ylab="w")
