...
\end{equation}
see [[refs -- two medical papers]] for more information. High values of NPI are bad, identifying those patients with very poor prognoses.

\subsubsection{Tumour grade}

Also look at the tumour grade. This was scored I-III on a scale of decreasing cell differentiation (ie. large numbers are bad). The same observations had missing values for NPI as had missing values for tumour grade.
...
Can use leave-one-out cross-validation to see how sensitive the results are to changes in the data. Using the ``best'' model above, the quasi-likelihood, the results from the leave-one-out cross-validation are not good (CV of about 10 for the MDS+DS and 2 for the lasso) indicating that the model was overfitting to the data. Working back through the models above, the best model appears to be the one with Normal errors.

To calculate the CV score, the following algorithm was used:
\begin{enumerate}
...
For the lasso model the CV was 1.56 and for the MDS+DS model the CV score was 2.0.
...
Above, the link function was determined by regressing the residuals onto the fitted values. To check that the link is reasonable, we can run this for each round of a cross-validation and see how the power fluctuates. This showed that over the 45 rounds of cross-validation, the power used in the link function varied between 0.06 and 3.1. The 3.1 appears to be an outlier, so using the mean value with that observation removed yields a link power of about 0.6.

Using this value, MDS+DS yields a CV score of 0.87, while the lasso is slightly better with a score of 0.69.
