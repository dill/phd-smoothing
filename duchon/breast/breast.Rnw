...
\begin{equation*}
\text{NPI}_i = f(x_{1i}, \dots, x_{di}) + \epsilon_i, \qquad  \epsilon_i \sim D
\end{equation*}
where $D$ is some distribution and $x_{1i}, \dots, x_{di}$ is the MDS projection of datum $i$ into $d$ dimensional MDS space. $f$ is some smooth function.
...
\subsection{Model 3 -- quasi-likelihood}
