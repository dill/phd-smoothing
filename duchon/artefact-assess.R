...
#   for(m.val in c(2,3,4)){
      ...
      mds.mod<-gam.mds(gendata.samp,gendata,bnd,mds.dim=mds.dim,bs="ds",m=c(2,mds.dim/2-1))
   
   pred.mat<-rep(NA,length(gendata.ind$x))
   pred.mat[ind]<-mds.mod$pred
   pred.mat<-matrix(pred.mat,50,50)
   image(xscale,yscale,pred.mat,main=paste(mds.dim,"D MDS s=",mds.dim/2-1,sep=""),asp=1,xlab="",ylab="",col=heat.colors(100),cex.axis=0.5)
   contour(xscale,yscale,pred.mat,add=T,labcex=0.3,lwd=0.5)
