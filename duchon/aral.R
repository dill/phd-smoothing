...
######################################################################
# plot setup
par(mfrow=c(2,3),las=1,mgp=c(1.5,0.75,0),mar=c(3,3,2,2),cex.axis=0.5,cex.lab=0.7)
...
# fit the model
mds3d.fit<-gam(chl~s(x,y,z,k=140,bs="ds",m=c(2,3/2-1)),data=aral.mds,family=Gamma(link="log"))

# do the prediction
mds3d.pred<-predict(mds3d.fit,newdata=pred.grid.mds,type="response")


pred.mat<-matrix(NA,gm,gn)
pred.mat[pred.onoff]<-mds3d.pred
