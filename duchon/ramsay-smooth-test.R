...
ramsay_smooth_test<-function(samp.size=250,noise.level=0.05,plot.it=FALSE){
   ...
   samp.data.mds<-data.frame(w=samp.mds[,3],x=samp.mds[,1],y=samp.mds[,2],z=samp.data$z)
   ...
   pred.size<-length(xx)
   pred.data.mds<-list(w=rep(0,pred.size),x=rep(0,pred.size),y=rep(0,pred.size))
   pred.data.mds$w[samp.ind]<-samp.data.mds$w  
   pred.data.mds$w[-samp.ind]<-pred.mds[,3]
   pred.data.mds$x[samp.ind]<-samp.data.mds$x 
   pred.data.mds$x[-samp.ind]<-pred.mds[,1]
   pred.data.mds$y[samp.ind]<-samp.data.mds$y
   ...
   ### Duchon
   b.mapped<-gam(z~s(x,y,w,k=100,m=c(2,3/2-1),bs="ds"),data=samp.data.mds)
