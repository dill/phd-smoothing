...
# pull out the 40 genes that best classify TEL according to chisq
TEL<-read.csv(file="TEL-chisq-genes.csv",header=FALSE)
TEL<-as.matrix(as.character(levels(as.factor(TEL[,1]))))
TEL.cols<-as.matrix(apply(TEL,1,grep,x=colnames(leuk)))
...
selected.cols<-c(TEL.cols,sample(total.cols,100))
   ...
   if(!is.null(b.gcv) & (class(b.gcv)!="try-error")){
      ...
      if(!is.null(b.ml) & (class(b.ml)!="try-error")){
         ...
         if(class(cvmin.lasso)!="try-error"){
            ...
            b.lasso<-try(glmnet(leuk.samp,type.samp,lambda=cvmin.lasso$lambda.min,family="binomial"))
