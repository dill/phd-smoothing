...
surf$z<-surf$z+5*bivn.kde.4$z-20*bivn.kde.5$z


bivn.6 <- mvrnorm(2500, mu = c(-2.6, 0.2), Sigma = matrix(c(0.1, 0, 0, 0.1), 2))
bivn.kde.6 <- kde2d(bivn.6[,1], bivn.6[,2], n =resolution, lims=lims)
bivn.7 <- mvrnorm(2500, mu = c(-2.2, -1), Sigma = matrix(c(0.1, 0, 0, 0.1), 2))
bivn.kde.7 <- kde2d(bivn.7[,1], bivn.7[,2], n =resolution, lims=lims)
bivn.8 <- mvrnorm(2500, mu = c(-2.7, 1.5), Sigma = matrix(c(0.1, 0, 0, 0.1), 2))
bivn.kde.8 <- kde2d(bivn.8[,1], bivn.8[,2], n =resolution, lims=lims)

surf$z<-surf$z+5*bivn.kde.6$z+5*bivn.kde.7$z+5*bivn.kde.8$z
...
write.csv(list(x=xx,y=yy,z=as.vector(surf$z),inside=ind),"wt2truth-new.csv",row.names=FALSE)
