...
sc.map.forwards<-function(points,nvertices,betam,nptsq,qwork,accuracy=as.single(0.0001),prevertices,polyvertices,complex.scale.factor,centre){
   ...
   mapint.call<-.Fortran("mapint",mode=as.integer(2),dpoints=points,npoints=as.integer(length(points)),n=as.integer(nvertices),betam=as.single(betam),nptsq=as.integer(nptsq),qwork=as.single(qwork),qsize=as.integer(length(qwork)),accuracy=as.single(accuracy),dz=prevertices,dw=polyvertices,dc=complex.scale.factor,dwc=centre,dretpoints=evaluated.points,reterrors=as.integer(rep(0,length(points))))
...
sc.map.backwards<-function(points,nvertices,betam,nptsq,qwork,accuracy=as.single(0.0001),prevertices,polyvertices,complex.scale.factor,centre){
   ...
   mapint.call<-.Fortran("mapint",mode=as.integer(1),dpoints=points,npoints=as.integer(length(points)),n=as.integer(nvertices),betam=as.single(betam),nptsq=as.integer(nptsq),qwork=as.single(qwork),qsize=as.integer(length(qwork)),accuracy=as.single(accuracy),dz=prevertices,dw=polyvertices,dc=complex.scale.factor,dwc=centre,dretpoints=evaluated.points,reterrors=as.integer(rep(0,length(points))))
