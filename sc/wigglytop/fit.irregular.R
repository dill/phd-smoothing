...
mses<-list(tprs=c(),sc=c(),soap=c())
# for testing, want these to be set at 7,20 and 10,40 resp.
intknots<-7
boundaryknots<-20
...
#   contour(pred.grid.tprs,add=T)
   ...
   # setup knots
   # this is a faff
   knots.x<-rep(seq(-2.9,2.9,length.out=intknots),intknots)
   knots.y<-rep(seq(-1.9,3.6,length.out=intknots),rep(intknots,intknots))
   ...
   # fit
   b.soap<-gam(z~s(x,y,bs="so",xt=list(bnd=list(verts)),k=boundaryknots),data=samp.data,knots=knots)
...
#   contour(axis.vals$x,axis.vals$y,pred.grid.soap,add=T)
   ...
   mses$sc<-c(mses$sc,mean((true.vals$z[true.vals$inside==1]-fv)^2,na.rm=T))
   mses$tprs<-c(mses$tprs,mean((true.vals$z[true.vals$inside==1]-fv.tprs)^2,na.rm=T))
   mses$soap<-c(mses$soap,mean((true.vals$z[true.vals$inside==1]-fv.soap)^2,na.rm=T))
}   
   cat("sc+tprs",mean(mses$sc),"\n")
   cat("tprs",mean(mses$tprs),"\n")
   cat("soap",mean(mses$soap),"\n")
