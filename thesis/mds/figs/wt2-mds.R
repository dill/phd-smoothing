...
#source("mds.R")
library(msg)
bnd <- read.csv("../../../mds/wt2-verts.csv",header=FALSE)
names(bnd)<-c("x","y")

# data
gendata <- read.csv("../../../mds/wt2truth.csv",header=TRUE)
...
## plot it
par(mfrow=c(1,2),las=1,mar=c(2.5,2.8,1.8,1.5),cex.axis=0.5,cex.lab=0.75,mgp=c(1.5,0.75,0))
plot(gendata$x,gendata$y,asp=1,pch=".",xlab=expression(x[1]),ylab=expression(x[2]),cex=0.7)
lines(bnd,lwd=2)
plot(mds2$points[,1],mds2$points[,2],asp=1,pch=".",xlab=expression(x[1]^"*"),ylab=expression(x[2]^"*"),cex=0.7)
dev.off()
