...
# send to a pdf...
pdf(file="1dadjust.pdf",height=4,width=5)

par(mfrow=c(2,2),las=1,mar=c(3,3,0.5,0.5),cex.axis=0.5,cex.lab=0.7,mgp=c(1.5,0.75,0))
plot(x=newdat$x,y=predict(b,newdat),main="",type="l",xlim=c(0,1),col="black",xlab="x",ylab="y")
...
plot(x=x.m,y=y,main="",type="l",xlim=c(0,max(x.m)),col="black",xlab="x*",ylab="y")
...
plot(x=newdat.m$x,y=predict(b.fix,newdat.m),main="",type="n",xlim=c(0,max(x.m)),xlab="x*",ylab="y")
...
#adjusted fit (green), data (black),\n unadjusted fit (blue)
plot(x=newdat$x,y=predict(b,newdat),main="",type="n",xlim=c(0,1),xlab="x",ylab="y")
