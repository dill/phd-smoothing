...
\begin{equation}
\mathfrak{F} g(\boldsymbol{\tau}) = \int_{\mathbb{R}^n} e^{2 \pi \sqrt{-1} \mathbf{x} \cdot \boldsymbol{\tau}} g(\mathbf{x}) \text{d}\mathbf{x}.
\end{equation}
...
Duchon suggests using the Fourier transform of the derivatives in the penalty, thus penalising those second derivatives that have particularly high frequencies. He also suggests putting a weighting factor in the integral. 
