...
In the two cases above, moving into three dimensions allowed \mdsap\ more accurately reproduce the true function. Counteracting the phenomenon above, the extra dimension allows us to see the ``width'' of part of the domain that we take a side view of the in the 2- dimensional projection.

It seems logical then to take a 3-dimensional projection in general to solve this problem without the need for penalty adjustments. However, this is not the case. \Fig{mds-comb} shows a long domain with three peninsulae at each end. The MDS projection of the domain in two dimensions gives the points in \fig{mds-comb-2d}. There are only four peninsulae in this figure (three corresponding to the three largest peninsulae in \fig{mds-comb}, one for the others), not the six which were in the original. Assuming that there is some information in the smaller peninsulae and there is reason to believe that leakage would be undesirable, this looks rather bad from a smoothing perspective.
...
So, in this case it is not unreasonable to think that an extra dimension could help. Taking a 3-dimensional projection, \fig{mds-comb-3d} is produced. Looking at these plots, and using \texttt{rgl} in \textsf{R} to further investigate, shows that there is still no separation of the smaller peninsulae.
...
Finally, moving into four dimensions (\fig{mds-comb-4d}) we begin to see separation in the smaller peninsulae. Unfortunately though, this is rather hard to discern given the nature of visualising 4-dimensional objects. Looking at eigenvalues of $D$ in order of magnitude, the largest is of the order of $10^3$, however the next are of the order $10^{-1}$ and continue getting smaller. Clearly the distance between the two groups of peninsulae (along the baseline of the figure) is dominating here.
...
From this it is safe to conclude that there are situations in which separation of the different parts of the domain cannot be guaranteed. One can imagine the case in which more peninsulae are added to \fig{mds-comb} and further distances, requiring higher-dimensional projections.

Although moving into continually higher dimensions seems appealing, practically it is rather more taxing. As the dimension of the problem is increased, as does the order of the penalty in the GAM (see \secref{GAMpenalties}). As this happens, there are more and more functions in the nullspace of the penalty, meaning that more functions may be used unpenalised. This is not desirable if we wish for a simple model which will run quickly.
