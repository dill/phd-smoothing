...
Following the work of the previous chapter, the objective is now to find a mapping into a space which causes minimum distortion to the density of space, whilst also effectively moving apart those parts of the domain which are subject to leakage. Dictating the final, transformed, domain from the outset will not be a flexible enough approach to work in general. The transformation should be to a domain that adapts itself to the domain in which the data lies.
...
\item \emph{\mdsap}: after transform a bivariate thin plate spline with basis size 100 was fit. The initial MDS grid was 74 points square.
...
\begin{enumerate}
\item \emph{Thin plate spline}: 
\item \emph{Soap film smoother}:
\item \emph{\mdsap}: 
\end{enumerate}
