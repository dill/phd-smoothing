...
Figure [[BLAH]] shows a plot of GCV score for 60 simulations from the wiggly top domain with a slightly different surface (as shown in figure [[figure of new wt2]]. The surface has three smaller peaking running down the left peninsulae, these were added to increase the complexity of the problem so that  [[ITS A DIFFERENT WT2, MENTION THIS!]] The boxplots are grouped according to the dimension of the MDS projection used, a loess [[cite Cleveland]] smooth is superimposed. The graph shows that there is a (albeit fairly shallow) minima in the score when the dimension is six.
