...
This penalty is a particular case of a more general result. \cite{duchon77} suggests using the Fourier transform [[ref?]] of the derivatives and pre-multiplying the norm (ie. the summation in the above expression) by a function of the variables in the frequency domain [[check]] and a constant $s$. 

Defining the Fourier transform of some function, $g$, in the usual way:
\begin{equation}
\mathfrak{F} g(\tau) = \int_{-\infty}^\infty e^{2 \pi i x \tau} g(x) \text{d}x,
\end{equation}
where $i=\sqrt{-1}$. [[MORE BLURB HERE]] For our purposes it only serves to remember that the Fourier transform decomposes functions into their frequency domain representations. [[BETTER]]
...
\subsubsection{Using GCV/ML scores}

Since the models we are fitting are relatively cheap compared to the cost of finding the within-area distances, GCV or ML scores can be used to find an optimal dimension for the MDS projection.
