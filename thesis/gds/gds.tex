...
\end{equation}
the function $w$ can then be used to pick out particularly high frequencies and penalise those more than the lower frequency ones. Setting $w(\boldsymbol{\tau})=1, \forall \boldsymbol{\tau}$ recovers the usual thin plate penalty in (\ref{tprs-pen}).
...
Combining Duchon splines with high dimensional MDS projections allowed points in the domain to be separated based on their within-area distances, while maintaining their ordering. This avoided both leakage and the artefacts seen in \secref{aral-sec}. \mdsds\ also beat the soap film smoother in MSE terms and provided comparable maps on real data.
