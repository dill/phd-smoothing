...
First re-iterating and expanding on the description of thin plate splines given in \secref{GAMtprs}, the thin plate spline penalty (as originally given in (\ref{tprs-pen})) in $d$ dimensions with derivative order $m$ is :
\begin{equation}
...
Duchon suggests the use of $w(\boldsymbol{\tau})= \lvert \boldsymbol{\tau} \rvert^{2s}$ for some choice of $s$\label{cor-5s5}:
\begin{equation}
