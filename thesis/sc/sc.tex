...
The forwards map, $\varphi$, is determined up to translation, scaling, and rotation by the prevertices (see below). \label{cor-3s13-1}The \emph{\sch\ parameter problem} is the task of efficiently finding the prevertices (hence $\varphi$). This is a complex problem since the prevertices are solutions to non-linear equations. The \sch\ parameter problem is discussed in \secref{sc-mapping-problem}. For the rest of this section the prevertices are assumed to be known. 

The rest of the section discusses the mappings for the upper half-plane, unit disc and rectangle. In each case the mapping takes the form of an integral which does not have a closed form, so it is important that the integrals can be computed quickly and accurately using quadrature. Accurate and fast computation of these integrals is covered in \citeb[pp. 27--29]{driscoll} and implemented in the \textit{SC Toolbox} package for MATLAB.
