...
This kind of approach is appealing since it allows the use of existing techniques in the transformed domain. It also seems natural to treat the domain as if it were made of silly putty and simply squash the region into the shape required to perform analysis.
...
F_k=\frac{\vert \varphiinv(w_{k+1}) -  \varphiinv(w_k) \vert}{\vert \varphiinv(w_2)-\varphiinv(w_1)\vert} - \frac{\vert w^*_{k+1} - w^*_k\vert}{\vert w^*_2 - w^*_1\vert}, \qquad \text{for } k=3,\dots,K-1,
 \end{equation*}
\item Letting $\mathbf{F}$ be the vector of $F_k$s, use steepest descent and then Newton's method to minimize each of the $F_k$s until $\vert \mathbf{F}\vert < \epsilon \quad \forall k$. \item Calculate $C$ and $A$ as detailed above.
