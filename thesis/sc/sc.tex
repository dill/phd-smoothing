...
The rest of the section discusses the mathematical form of the mappings for the upper half-plane, unit disc and rectangle. In each case the mapping takes the form of an integral which does not have a closed form, so it is important that the integrals can be computed quickly and accurately using quadrature. Accurate and fast computation of these integrals is covered in \citeb[pp. 27--29]{driscoll} and implemented in the \textit{SC Toolbox} package for MATLAB written by Tobin A. Driscoll.
