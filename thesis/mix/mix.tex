...
The core principle here is to replace the ``key function plus adjustment terms'' model for the detection function with a mixture model. The simplest example would be to define $g$ as some finite weighted sum of half-Normal distributions:
\begin{equation}
...
\end{align}
Where $g_j(x_i;\sigma_j) = \exp \Big( \frac{-x_i^2}{2 \sigma_j^2}\Big)$.

The above expression could also be thought of as decomposing $f$ into mixture parts ($f_j$), yielding the same expression:
\begin{align}
\mathcal{L}(\bm{\sigma}, \bm{\pi}; \bm{x}) &= \prod_{i=1}^n f(x_i;\bm{\sigma}, \bm{\pi}),\\
...
\begin{equation}
\mu(\bm{z}_i) = \sum_{j=1}^J \int_0^w g_j(x; \sigma_j(\bm{z}_i;\bm{\beta}_j)) \text{d}x.
\end{equation}
...
\subsection{Per observation detectability}
When using covariates, each object effectively has its own detection function and hence its own per observation detectability. Calculating this is fairly simple. Starting from the definition of detectability given in equation (3.19) of Buckland \textit{et al} (2004) p 38 (modified slightly for notational consistency), the detection function is replaced with the mixture model version:
\begin{align*}
P_a(\bm{z}_i) =& \frac{\mu(\bm{z}_i)}{w},\\
=& \frac{1}{w} \int_0^w g(x; \sigma(\bm{z}_i,\bm{\theta})) \text{d}x,\\
=& \frac{1}{w} \int_0^w \sum_{j=1}^J \hat{\pi}_j g_j(x; \sigma_j(\bm{z}_i, \bm{\beta}_j)) \text{d}x,\\
=& \frac{1}{w} \sum_{j=1}^J \hat{\pi}_j \int_0^w  g_j(x; \sigma_j(\bm{z}_i, \bm{\beta}_j)) \text{d}x,
\end{align*}
so the detectability can now be calculated for each observation.


\subsection{Average detectability}

If we wish to calculate the average detectability, then we must first find the population mixture proportions and use these in order to calculate the overall average detectability. The estimated population mixture proportions, $\dot{\pi}_j$ (for $j=1\dots J$ are then defined as:
\begin{equation}
...
Once the $\dot{\pi}_j$s are found, they can be used in place of the $\hat{\pi}_j$s in order to calculate the average detectability.
...
Outline of the EM algorithm is taken from the very helpful set of notes by \cite{piater}. As is usual with EM, expectation and maximisation steps are looped until convergence. Letting $f_j(x_i;\sigma_j(\bm{z}_i,\bm{\beta}_j))$ be the PDF of mixture part $j$ evaluated at distance $x_i$ with covariates $\bm{z}_i$, The expectation and maximisation steps are defined as follows.
\begin{itemize}
...
Keeping the $\pi_j$s fixed (from the previous step), optimise with respect to the $\sigma_j$s (or, rather, $\beta_{jk}$s). Here BFGS is used with analytic gradients. 
\end{itemize}
...
\cite{beavers98} give a method for estimating the starting values for the scale parameter for a half-Normal detection function. In the non-covariate case, the estimate is given as the intercept parameter from intercept only regression on $\log(x+\frac{w}{1000})$ (where $w$ denotes the truncation distance, as above). For covariate models, the equation used for the $\sigma$ is used in the regression and the estimated parameters from the linear regression are used as the starting values for the $\beta$s.
