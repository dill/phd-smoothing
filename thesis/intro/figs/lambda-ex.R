...
#points(dat$x2,dat$y,pch=".")


# optimal
b<-gam(y~s(x2,k=100),data=dat)
plot(b,se=FALSE,rug=FALSE,residuals=TRUE,ylab=expression(hat(f)(x)),xlab="x")
lines(true$x,true$y,type="l",col="blue")


# lambda=0
b.0<-gam(y~s(x2,k=100),data=dat,sp=0)
plot(b.0,se=FALSE,rug=FALSE,residuals=TRUE,ylab=expression(hat(f)(x)),xlab="x")
lines(true$x,true$y,type="l",col="blue")


# lambda=infinity
b.inf<-gam(y~s(x2,k=100),data=dat,sp=1e10)
plot(b.inf,se=FALSE,rug=FALSE,residuals=TRUE,ylab=expression(hat(f)(x)),xlab="x")
lines(true$x,true$y,type="l",col="blue")
