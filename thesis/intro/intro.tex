...
\hat{D} = \frac{n}{2wL}.
\end{equation}
Now consider the case in which we are not certain that we observe everything in our strips. Then, what we might do is replace the fixed width $w$ with an estimated \textit{effective strip width}, $\hat{\mu}$. The effective strip width is the point at which we miss as many individuals within a distance of $\hat{\mu}$ from the line, as are observed beyond $\hat{\mu}$. Density can then be expressed as:
\begin{equation}
