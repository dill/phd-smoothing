...
\label{intro-GCV}
\end{equation}
where $\text{tr}(\mathbf{A})$ indicates the trace of $\mathbf{A}$, the hat (or influence) matrix for the smoother (see \secref{GAMEDF}). Numerical minimization of $\mathcal{V}_g$ gives the optimal model parameters and smoothing parameter ($\bm{\hat{\beta}}$ and $\hat{\lambda}$). Further details are given in \citeb[pp.  134-137]{simonbook}. 
