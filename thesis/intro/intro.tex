...
This section gives a very brief introduction to generalized additive models with the emphasis on moving from the linear and generalized linear model to additive and generalized additive models in a finite-area smoothing scenario. This only touches on the surface of what is a vast literature. Excellent (and much more complete) introductions to the topic are given in \cite{simonbook} and \cite{rwc}.

\subsection{From GLMs to GAMs}

Generalized linear models (GLMs) have been around for almost [[TKTKTK X]] years, in that time they have become a standard tools for those performing statistical analyses. The idea is simple, one has measure some response $Z_i$ (for $i=1,\ldots,n$), and $p$ covariates (in a $n\times p$ matrix $X$) believed to effect the response. In a GLM, the relationship between the response and predictors is
\begin{equation}
g(\mu_i) = \beta_0 + X_i\bm{\beta} \quad \text{for } i=1,\cdots,n,
\end{equation}
where $g$ is a link function, such that $\mu_i = \mathbb{E}[Z_i]$ (where $Z_i$ has an exponential family distribution). $X_i$ is the $i^\text{th}$ row of the matrix $X$.
...
\begin{equation}
g(\mu_i) = X^*_i\bm{\beta} + \sum_{j=1}^J s_j(x_{ji}) \quad \text{for } i=1,\cdots,n,
\end{equation}
where $g$ is a link function (as in the GLM case), such that $\mu_i = \mathbb{E}[Y_i]$ (where $Y_i$ is a response variable with an exponential family distribution). $X^*_i$ is a row of the matrix $X^*$ which contains the covariates which enter the model in a linear way. The $s_{j}$s are smooth functions (see below) and the $x_{ji}$ are covariates which are related to the response in some non-linear way.
...
2d smoothing
	tensors
	isotropy -- in particular with reference to spatial smoothing
	spatio-temporal as an extension
...
%A large class of statistical problems can thought of as finding the relationship between observations of a response (denoted $z_i$) and a predictor ($x_i$). The $z_i$ is a realization of some random variable $Z$. The most basic case of this is the linear or regression model, which can be written:
%\begin{equation}
