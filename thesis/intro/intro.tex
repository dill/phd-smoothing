...
DO SOMETHING WITH THIS SENTENCE! Note that $\bm{z}$ is always a $K$-vector, those observations with no value of covariate take the value zero and thus have no effect on the model.
...
With this formulation, abundance can be estimated by a Horvitz-Thomspon-like estimator (CITE Sampling book, THOMPSON, \cite{ADS}, p.23):
\begin{equation}
...
Recently developments, particularly in mark-recapture (D+R, Pledger, Byron) have shown that mixture models can be an extremely useful and flexible method of modelling...

no monotonicity problems
