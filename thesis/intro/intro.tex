...
\caption{Given a population (the dots), an example of the performance of quadrat sampling (left), strip transect sampling (middle), and distance sampling (right). In the first two cases, the grey boxes represent the sampling units. Note that there are many observations just outside of the boxes, which cannot be recorded by survey staff. In the distance sampling case, the solid vertical lines represent the transects and the dashed line gives an (arbitrary) cutoff of the distance to which the observer can see. Distances are shown by the solid horizontal lines.}
\label{quad-to-ds}
\end{figure}
...
\label{ds-D}
\end{equation}
Here $w$ is the point after which observations are discarded and is referred to as the \textit{truncation distance}. Truncation is used to discard outliers that make the estimation process tricky.
...
\cite{buckland92} gives a ``key function plus adjustment term'' formulation for the detection function...
