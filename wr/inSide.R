...
in.poly<-function( poly, x,y){
   ind1<-in.poly1(matrix(c(poly$x,poly$y),length(poly$x),2),
                  matrix(c(x,y),length(x),2))
   ind2<-in.poly1(matrix(c(-poly$x,-poly$y),length(poly$x),2),
                  matrix(c(-x,-y),length(x),2))
