...
wt2_smooth_test<-function(samp.size=250,noise.level=0.05,plot.it=FALSE){
   ...
   m<-45;n<-45
   xm <- seq(1,3.5,length=m);yn<-seq(-3,1,length=n)
   ...
   dense<-list(x=c(gendata$x,xx),
               y=c(gendata$y,yy))

   ### do the PCO and construct the data frame
   # create D
   D<-create_distance_matrix(dense$x,dense$y,bnd)
   ...
   samp.mds<-insert.mds(gendata.samp,dense,mds,bnd)
